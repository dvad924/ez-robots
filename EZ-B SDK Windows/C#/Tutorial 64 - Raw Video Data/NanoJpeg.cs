...
38, 31, 39, 46, 53, 60, 61, 54, 47, 55, 62, 63 };
    ...
    public byte GetByteAtIndex(int index) {
      ...
      return ((byte*)Image + index)[0];

      //byte* ptr = (byte*)Image + index3;
    ...
    public byte GetPixelBlue(int x, int y) {
      ...
      byte* ptr = (byte*)Image + y * Stride + x * 3;
    ...
    public AForge.Imaging.UnmanagedImage GetAsUnmanagedImage {
      get {

        return new AForge.Imaging.UnmanagedImage((IntPtr)Image, Width, Height, Stride, System.Drawing.Imaging.PixelFormat.Format24bppRgb);
      }
    }
    ...
    /// </summary>
    /// <param name="jpeg"></param>
    public void Decode(byte[] jpeg, bool flip) {
      fixed (byte* ptr = jpeg) {
        Decode(ptr, jpeg.Length, flip);
      }
    }
    ...
    public void Decode(byte* jpeg, int size, bool flip) {
      ...
      if (this.size < 2)
        throw new NJException(NJErrorCode.NoJpeg);

      if (((this.pos[0] ^ 0xFF) | (this.pos[1] ^ 0xD8)) != 0)
        throw new NJException(NJErrorCode.NoJpeg);
      ...
      while (!IsDone) {
        ...
        if ((this.size < 2) || (this.pos[0] != 0xFF))
    ...
    private void ColIDCT(int* blk, byte* outv, int stride) {
      ...
      *outv = Clip(((x7 + x1) >> 14) + 128);
      outv += stride;
      *outv = Clip(((x3 + x2) >> 14) + 128);
      outv += stride;
      *outv = Clip(((x0 + x4) >> 14) + 128);
      outv += stride;
      *outv = Clip(((x8 + x6) >> 14) + 128);
      outv += stride;
      *outv = Clip(((x8 - x6) >> 14) + 128);
      outv += stride;
      *outv = Clip(((x0 - x4) >> 14) + 128);
    ...
    private void DecodeSOF() {
      ...
      for (i = 0, c = this.comp; i < this.ncomp; ++i, ++c) {
        ...
        if ((c->ssx = this.pos[1] >> 4) == 0)
          throw new NJException(NJErrorCode.SyntaxError);

        if ((c->ssx & (c->ssx - 1)) != 0)
          throw new NJException(NJErrorCode.Unsupported);  // non-power of two

        if ((c->ssy = this.pos[1] & 15) == 0)
          throw new NJException(NJErrorCode.SyntaxError);

        if ((c->ssy & (c->ssy - 1)) != 0)
          throw new NJException(NJErrorCode.Unsupported);  // non-power of two

        if (((c->qtsel = this.pos[2]) & 0xFC) != 0)
          throw new NJException(NJErrorCode.SyntaxError);
      ...
      if (this.ncomp == 3)
        this.rgb = (byte*)Marshal.AllocHGlobal(this.width * this.height * this.ncomp);
    ...
    private void DecodeScan() {
      ...
      if (this.length < (4 + 2 * this.ncomp))
        throw new NJException(NJErrorCode.SyntaxError);
      if (this.pos[0] != this.ncomp)
        throw new NJException(NJErrorCode.Unsupported);
      Skip(1);
      for (i = 0, c = this.comp; i < this.ncomp; ++i, ++c) {
        if (this.pos[0] != c->cid)
          throw new NJException(NJErrorCode.SyntaxError);
        if ((this.pos[1] & 0xEE) != 0)
          throw new NJException(NJErrorCode.SyntaxError);
      ...
      if (this.pos[0] != 0 || this.pos[1] != 63 || this.pos[2] != 0)
        throw new NJException(NJErrorCode.Unsupported);
    ...
    private void ConvertYCC(bool flip) {
      ...
      for (i = 0, c = this.comp; i < this.ncomp; ++i, ++c) {
        ...
        if ((c->width < w) || (c->height < h))
          throw new NJException(NJErrorCode.InternalError);
      }
